...
def ppp_stuff(array, create_copy=False):
    """PPP byte stuffing for framing.

    Stuffing involves adding a FRAME_CHAR 0x7E '~' to the beginning and end of
    a frame and XOR'ing any bytes with MASK_CHAR 0x20 that equal the FRAME/ESC
    char. This allows detection of frame boundaries without confusion.
    """
    if create_copy:
        array = bytearray(array)  # no .copy() in 2.7
    ...
    # Insert ESC char in front of masked char (reverse order to avoid shifting)
    for i in sorted(ind, reverse=True):
        array.insert(i, ESC_CHAR)
...
class PPPUnstuff(object):
    ...
    def add_to_buffer(self, byte):
        if self.idx >= self.buffer_size:
            if getattr(config, "write_log", False):
                logging.warning("Exceeded maximum buffer size")
            self.idx = 0
        else:
            self.buffer[self.idx] = byte
    ...
    def unstuff_byte(self, byte):
        """Stateful byte parser for unstuffing PPP stuffed frames.

        Unstuffing requires:
        - Removing the FRAME_CHAR (0x7E) at start/end
        - Handling ESC_CHAR (0x7D): XOR following byte with MASK_CHAR (0x20)
        """
        # If frame char, assume at the end of the frame, if no data, pass
        if byte == FRAME_CHAR:
            # Ignore leading junk bytes like 0 from RS485 adapters
            if self.idx > 2:
                idx_copy = self.idx
                self.idx = 0
                return bytearray(self.buffer[0:idx_copy])  # direct slice
            else:
                self.idx = 0
                return None
