...
class Type1Packet(AbstractPacket):
    def __init__(self, buffer):
        ...
        if len(buffer) == self.size_lookup[0]:
            (
                self.header,
                self.pos[0], self.cur[0],
                self.pos[1], self.cur[1],
                self.pos[2], self.cur[2],
                self.pos[3], self.cur[3],
                self.pos[4], self.cur[4],
                self.pos[5], self.cur[5],
            ) = struct.unpack("<bhhhhhhhhhhhh", buffer[0:25])

            idx = 0
            for i in range(25, 70, 3):
                self.fsr[idx] = struct.unpack("<H", buffer[i:i+2])[0] & 0x0FFF
                self.fsr[idx + 1] = (struct.unpack("<H", buffer[i+1:i+3])[0] & 0xFFF0) >> 4
                idx += 2

            self.hot_cold = struct.unpack("<b", buffer[-1:])[0]
            if self.hot_cold:
                if config.write_log:
                    logging.warning("Collision Detected: {}".format(self.hot_cold))
            ...
            if config.write_log:
                logging.debug("Bad sized Type 1 Frame: {}".format(buffer))
...
class Type2Packet(AbstractPacket):
    def __init__(self, buffer):
        ...
        if len(buffer) == self.size_lookup[1]:
            ...
            if config.write_log:
                logging.debug("Bad sized Type 2 Frame: {}".format(buffer))


class Type3Packet(AbstractPacket):
    def __init__(self, buffer):
        super(Type3Packet, self).__init__()
        if len(buffer) == self.size_lookup[2]:
            (
                self.header,
                self.pos[0], self.cur[0],
            ...
            if config.write_log:
                logging.debug("Bad size type 3 frame: {}".format(buffer))


def parse_packet(byte_array):
    """Takes in un-stuffed frames, checks reply mode and checksum then converts
    to class derived from AbstractPacket"""
    if len(byte_array) == 1:
        return None  # TODO 0x00 is a valid frame that has a reply mode and passes

    reply_mode = byte_array[0] & 0x0F

    if reply_mode not in (0, 1, 2):
        if config.write_log:
            logging.warning("Unknown packet received {}".format(byte_array))
        return None

    checksum = byte_array.pop()
    expected = (-sum(byte_array)) & 0xFF

    if checksum != expected:
        if config.write_log:
            logging.warning("Checksum failed on {}".format(byte_array))
            logging.warning("Anticipated {} Received {}".format(expected, checksum))
        return None

    # Parse packet based on reply_mode index
    packet = (Type1Packet, Type2Packet, Type3Packet)[reply_mode](byte_array)
