...
public class DisconnectDaemonInstallationNodeContribution implements InstallationNodeContribution {
	private static final String POPUP_TITLE_KEY = "popuptitle";
	private static final String XMLRPC_VARIABLE = "Disconnect_daemon";
	...
	private final DisconnectDaemonInstallationNodeView view;
	...
	public DisconnectDaemonInstallationNodeContribution(InstallationAPIProvider apiProvider,
												DisconnectDaemonInstallationNodeView view,
												DataModel model,
												DisconnectDaemonService daemonService,
	...
	private void applyDesiredDaemonStatus() {
		new Thread(new Runnable() {
			@Override
			public void run() {
				if (DisconnectDaemonInstallationNodeContribution.this.isDaemonEnabled()) {
					// Download the daemon settings to the daemon process on initial start for real-time preview purposes
					try {
						DisconnectDaemonInstallationNodeContribution.this.awaitDaemonRunning();
						// daemonStatusMonitor.setTitle(DisconnectDaemonInstallationNodeContribution.this.getPopupTitle());
					} catch (Exception e) {
