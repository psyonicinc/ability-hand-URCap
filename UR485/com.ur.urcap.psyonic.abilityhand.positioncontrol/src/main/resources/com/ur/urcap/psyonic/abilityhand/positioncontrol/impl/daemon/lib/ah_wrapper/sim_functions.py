...
class GeneratedPacket(object):
    def __init__(self, pos=None, cur=None, vel=None, fsr=None, reply_mode=None, mode=None):
        if pos is None:
            self.pos = [random.randint(0, 90) for _ in range(6)]
            self.pos[-1] *= -1
        else:
        ...
        self.packet = ppp_stuff(self.generate_packet())
        # Force integers in case `bytearray` returns str-like values in 2.7
        self.packet_list = [ord(i) if isinstance(i, str) else int(i) for i in self.packet]

    def generate_packet(self):
        msg = bytearray()
        msg.append(MODES_AND_VAR[self.mode][1][self.reply_mode])  # Byte 0 - Format Header
