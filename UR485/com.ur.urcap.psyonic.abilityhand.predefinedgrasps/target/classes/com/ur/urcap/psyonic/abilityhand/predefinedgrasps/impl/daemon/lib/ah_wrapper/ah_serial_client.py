...
from __future__ import print_function  # allow modern print() in Python 2.7
import logging
import threading
import time
import os
...
class AHSerialClient(object):
    def __init__(
        ...
        self._read_size = read_size
        self._cmd_lock = threading.Lock()
        self._wait_time_s = 1.0 / rate_hz  # float division
        self._write_thread = None
        self._read_thread = None
        self._reading = False
        self._writing = False
        ...
        # Other classes
        self.hand = Hand(addr=hand_address)
        self._unstuffer = PPPUnstuff(buffer_size=read_size)
        ...
        if read_thread:
            self._read_thread = threading.Thread(target=self._read_thread_loop)
        if write_thread:
            self._write_thread = threading.Thread(target=self._write_thread_loop)
        if auto_start_threads:
            self.start_threads()
                    ...
                    if frame:
                        ...
                        if parsed is not None and parsed.valid:
                            ...
                            try:
                                ...
                                if isinstance(parsed, Type3Packet):
                                    self.hand._update_cur(
                                        positions=parsed.pos,
                                        velocity=parsed.vel,
                                        current=parsed.cur,
                                    )
                                elif isinstance(parsed, Type2Packet):
                                    self.hand._update_cur(
                                        positions=parsed.pos,
                                        velocity=parsed.vel,
                                        fsr=parsed.fsr,
                                    )
                                elif isinstance(parsed, Type1Packet):
                                    self.hand._update_cur(
                                        positions=parsed.pos,
                                        current=parsed.cur,
                                        fsr=parsed.fsr,
                                    ...
                                    if config.write_log:
                                        ...
                                        logging.warning("Invalid msg {}".format(msg))
                            except Exception as e:
                                if config.write_log:
                                    logging.warning(str(e))

            time.sleep(self._wait_time_s / 2.0)
        self.end_time = time.time()
        if config.write_log:
            logging.info("Stopping Read Thread")
    ...
    def set_position(self, positions, reply_mode=None, addr=None):
        if reply_mode is None:
            reply_mode = self.reply_mode
        if addr is None:
            addr = self.hand.addr
        self.set_command(
            create_pos_msg(reply_mode=reply_mode, positions=positions, addr=addr)
        )
    ...
    def set_velocity(self, velocities, reply_mode=None, addr=None):
        if reply_mode is None:
            reply_mode = self.reply_mode
        if addr is None:
            addr = self.hand.addr
        self.set_command(
            create_vel_msg(reply_mode=reply_mode, velocities=velocities, addr=addr)
        )
    ...
    def set_duty(self, duties, reply_mode=None, addr=None):
        if reply_mode is None:
            reply_mode = self.reply_mode
        if addr is None:
            addr = self.hand.addr
        self.set_command(create_duty_msg(reply_mode=reply_mode, duties=duties, addr=addr))
        if not isinstance(duties, list):
            duties = [duties] * 6
            duties[-1] *= -1
        self.hand.update_tar(duties=duties)

    def set_torque(self, currents, reply_mode=None, addr=None):
        if reply_mode is None:
            reply_mode = self.reply_mode
        if addr is None:
            addr = self.hand.addr
        self.set_command(
            create_torque_msg(reply_mode=reply_mode, currents=currents, addr=addr)
        )
    ...
    def print_stats(self):
        if not self.end_time:
            ...
            end_time = self.end_time
        print(
            "Rate: {:.1f} \nWrites: {} \nReads: {} \nPacket Loss: {:.3f}%".format(
                ((self._conn.n_writes - 1) / (end_time - self.start_time)),
                self._conn.n_writes - 1,
                self.n_reads - 1,
                (1 - ((self.n_reads - 1) / float(self._conn.n_writes - 1))) * 100,
            )
        )
