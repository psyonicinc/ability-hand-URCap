...
public class ConnectionDaemonInstallationNodeContribution implements InstallationNodeContribution {
	private static final String POPUP_TITLE_KEY = "popuptitle";
	private static final String XMLRPC_VARIABLE = "Connect_daemon";
	...
	private final ConnectionDaemonInstallationNodeView view;
	...
	public ConnectionDaemonInstallationNodeContribution(InstallationAPIProvider apiProvider,
												ConnectionDaemonInstallationNodeView view,
												DataModel model,
												ConnectionDaemonService daemonService,
	...
	private void applyDesiredDaemonStatus() {
		new Thread(new Runnable() {
			@Override
			public void run() {
				if (ConnectionDaemonInstallationNodeContribution.this.isDaemonEnabled()) {
					// Download the daemon settings to the daemon process on initial start for real-time preview purposes
					try {
						ConnectionDaemonInstallationNodeContribution.this.awaitDaemonRunning();
						// daemonStatusMonitor.setTitle(ConnectionDaemonInstallationNodeContribution.this.getPopupTitle());
					} catch (Exception e) {
