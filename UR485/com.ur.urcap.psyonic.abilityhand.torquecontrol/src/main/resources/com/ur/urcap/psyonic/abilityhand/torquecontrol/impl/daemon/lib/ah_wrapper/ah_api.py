...
def create_misc_msg(cmd, addr=0x50):
    """Create misc message and stuff it (Python 2.7 compatible)"""
    msg = struct.pack("<BB", addr, cmd)
    msg = [ord(b) for b in msg]  # Convert packed bytes to list of ints
    msg.append(compute_chksum(msg))
    msg = ppp_stuff(msg)
    return msg
...
def create_pos_msg(reply_mode, positions, addr=0x50):
    ...
    msg = struct.pack("<BB", addr, 0x10 + reply_mode)
    msg = [ord(b) for b in msg]

    for p in positions:
        p_scaled = max(min(p * DEGREES_CONSTANT, LIMIT), -LIMIT)
        p_bytes = struct.pack("<h", int(p_scaled))
        msg.extend(ord(b) for b in p_bytes)
...
def create_torque_msg(reply_mode, currents, addr=0x50):
    ...
    msg = struct.pack("<BB", addr, 0x30 + reply_mode)
    msg = [ord(b) for b in msg]

    for a in currents:
        a_bytes = struct.pack("<h", int(a * C))  # Assuming C is a defined constant
        msg.extend(ord(b) for b in a_bytes)
...
def create_vel_msg(reply_mode, velocities, addr=0x50):
    if config.velocity_warning:
        print("WARNING VELOCITY TARGETS UNSTABLE AT THE MOMENT AND MAY CAUSE THUMB OSCILLATIONS, SUGGEST USING POSITION INSTEAD")
        config.velocity_warning = False
    ...
    msg = struct.pack("<BB", addr, 0x20 + reply_mode)
    msg = [ord(b) for b in msg]

    for v in velocities:
        v_bytes = struct.pack("<h", int(v * VELOCITY_CONSTANT))
        msg.extend(ord(b) for b in v_bytes)
...
def create_duty_msg(reply_mode, duties, addr=0x50):
    if isinstance(duties, (int, float)):
        duties = [duties] * 6
        duties[-1] *= -1
    ...
    msg = struct.pack("BB", addr, 0x40 + reply_mode)
    msg = [ord(b) for b in msg]

    for d in duties:
        d_mapped = map_value(d, -100, 100, -VOLTAGE_LIMIT, VOLTAGE_LIMIT)
        d_bytes = struct.pack("<h", int(d_mapped))
        msg.extend(ord(b) for b in d_bytes)
...
def create_write_reg_msg(val_addr, val, addr=0x50):
    msg = [ord(struct.pack("<B", addr)), ord(struct.pack("<B", 0xDE))]

    b2 = struct.pack("<i", val_addr)
    msg.extend(ord(b) for b in b2)
...
def create_read_reg_msg(val_addr, addr=0x50):
    msg = [ord(struct.pack("<B", addr)), ord(struct.pack("<B", 0xDA))]
